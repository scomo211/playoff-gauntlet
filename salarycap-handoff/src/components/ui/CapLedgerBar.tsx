...
export function CapLedgerBar({ cap, height = 13, className = '' }: CapLedgerBarProps) {
  const total = (cap.baseCap || BASE_CAP) + Math.max(0, cap.bonusCap);
  ...
  return (
    <div
      ...
      style={{ height }}
      role="img"
      aria-label={`Salaries ${cap.salaries}, dead cap ${cap.deadCap}, available ${cap.available}`}
    >
      <span className="block h-full bg-salary transition-[width] duration-200" style={{ width: pct(cap.salaries) }} />
      <span className="block h-full bg-flag transition-[width] duration-200" style={{ width: pct(cap.deadCap) }} />
